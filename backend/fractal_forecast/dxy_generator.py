"""
DXY Fractal Forecast Generator (P1 · fractal_native_v1)
========================================================
...
def generate_dxy_forecasts() -> int:
    ...
    if not forecast.get("ok"):
        print(f"[DXY NativeGen] degraded — reason={forecast.get('reason')}, skipping")
        return 0
    ...
    for horizon_key, horizon_days in STANDARD_HORIZONS.items():
        ...
        try:
            ...
            if "duplicate key" in str(e).lower() or "E11000" in str(e):
                continue
            print(f"[DXY NativeGen] Insert error ({horizon_key}): {e}")

    print(f"[DXY NativeGen] Generated {generated} forecasts ({today_bucket}) v={model_version} price={current_price:.2f}")
    return generated
