...
def create_router(module: "TelegramModule") -> APIRouter:
    ...
    async def feed_v2(
        ...
        hasMedia: bool = Query(None),
        minViews: int = Query(None),
        sortBy: str = Query("date"),
    ):
        return await module.get_feed(actorId, page, limit, windowDays, language, hasMedia, minViews, sortBy)
    ...
    async def channel_list(
        ...
        sortBy: str = "fomoScore",
        q: str = Query(None),
        language: str = Query(None)
    ):
        return await module.get_channels(limit, offset, sortBy, q, language)
    ...
    async def watchlist_clear(actorId: str = "default"):
        return await module.clear_watchlist(actorId)
    ...
    @router.get("/channels/search-telegram")
    async def search_telegram_channel(username: str = Query(...)):
        """Search for a channel on Telegram and return its info for adding"""
        return await module.search_telegram_channel(username)
    
    @router.post("/channels/add")
    async def add_channel(body: dict = None):
        """Add a new channel from Telegram to the tracking system"""
        username = (body or {}).get("username", "")
        return await module.add_channel_from_telegram(username)
    
    @router.post("/channel/{username}/analyze-product")
    async def analyze_product(username: str):
        """AI analysis of channel's product/monetization"""
        return await module.analyze_channel_product(username)
