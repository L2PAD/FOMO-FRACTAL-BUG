...
async def add_to_watchlist(db, username: str, actor_id: str = "a_public") -> Dict[str, Any]:
    ...
    try:
        ...
        # Invalidate cached alerts so they regenerate with new feed
        await db.tg_alerts.delete_many({})
...
async def clear_watchlist(db, actor_id: str = "default") -> Dict[str, Any]:
    """Remove all channels from watchlist for an actor"""
    try:
        result = await db.tg_watchlist.delete_many(
