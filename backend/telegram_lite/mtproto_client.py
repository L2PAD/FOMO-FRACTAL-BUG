...
class MTProtoClient:
    ...
    async def search_global(self, query: str, limit: int = 5) -> list:
        """Search Telegram globally for channels/groups by name"""
        if not await self.connect():
            return []
        try:
        ...
        try:
            ...
            async for msg in self._client.iter_messages(entity, limit=min(limit, 1000)):
                ...
                msg_data = {
                    ...
                    'pinnedInChannel': getattr(msg, 'pinned', False) or False,
                    'mediaType': None,
                    'mediaLocalPath': None,
                    'mediaSize': None,
