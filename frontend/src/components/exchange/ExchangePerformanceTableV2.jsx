/**
 * Exchange Forecast Performance Table V2
 * 
 * Structure (consistent across all horizons):
 *   Fixed Top 3: Yesterday / Today / Tomorrow
 *   Divider: "Pending"
 *   Remaining pending forecasts (scrollable for 30D)
 * 
 * Date column = Evaluate At, tooltip = Created at
 */
...
export default function ExchangePerformanceTableV2({ symbol = 'BTC', horizon = '7D', limit = 30 }) {
  ...
  const fetchData = useCallback(async () => {
    try {
      const res = await fetch(`${API}/api/market/exchange/performance-v2?symbol=${symbol}&horizon=${horizon}&limit=${limit}`);
