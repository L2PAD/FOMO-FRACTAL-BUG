...
/** Find the best matching row for a given date bucket — by forecast creation date (asOf/createdAt) */
function findRowForDate(rows, dateBucket) {
  return rows.find(r => (r.createdAt || r.asOf || '').slice(0, 10) === dateBucket) || null;
}
      ...
      <div className="flex items-center gap-5 mb-4 px-1" data-testid="perf-summary">
        ...
        <div className="flex items-center gap-1.5">
          ...
          <span className="tabular-nums" style={{ fontSize: 13, color: '#0f172a' }}>
            {(summary.evaluated ?? (summary.wins + summary.losses))}/{summary.total}
          </span>
        </div>
        {summary.overdue > 0 && (
          <div className="flex items-center gap-1.5">
