/**
 * Fractal Forecast Performance Table
 * 
 * Columns: Day | Eval At | Dir | Target | Error% | Δ Entry | Conf | Status
 * Features: Horizon tabs, confidence colors, status colors, tooltips
 */
...
export default function FractalPerformanceTable({ scope = 'BTC', horizon: initialHorizon = '7D', limit = 40 }) {
  ...
  const fetchData = useCallback(async () => {
    setLoading(true);
    try {
      const res = await fetch(`${API}/api/fractal/forecasts?scope=${scope}&horizon=${activeHorizon}&limit=${limit}`);
      if (!res.ok) throw new Error(`${res.status}`);
      const json = await res.json();
      if (json.ok) setData(json);
    } catch {}
    setLoading(false);
  }, [scope, activeHorizon, limit]);
  ...
  return (
    <div data-testid="fractal-performance-table" className="mt-4">
      {/* Horizon Tabs */}
      <div className="flex items-center gap-1 mb-3" data-testid="fractal-horizon-tabs">
      ...
      {loading ? (
        ...
        <>
          {/* Summary bar */}
          <div className="flex items-center gap-5 mb-3 px-1" data-testid="fractal-perf-summary">
            <div className="flex items-center gap-1.5">
              <span style={{ color: '#64748b', fontSize: 12 }}>Win Rate</span>
              <span className="font-bold tabular-nums" data-testid="fractal-perf-win-rate"
            ...
            <div className="flex items-center gap-1.5">
              ...
              <span className="font-bold tabular-nums" data-testid="fractal-perf-avg-error"
          ...
          <div className="overflow-auto rounded-lg" style={{ maxHeight: 420, border: '1px solid rgba(15,23,42,0.06)' }}>
            <table className="w-full text-[13px]" data-testid="fractal-perf-table">
              <thead>
                ...
                {pendingRows.length > 0 && (
                  <tr>
                    <td colSpan={8} className="py-2 px-3" style={{ borderTop: '1px solid rgba(15,23,42,0.08)' }}>
                      <span style={{ fontSize: 11, color: '#94a3b8', fontWeight: 500 }} data-testid="fractal-pending-divider">
...
function ForecastRow({ row, isPending }) {
  ...
  return (
    <tr className="transition-colors hover:bg-slate-50/50"
      ...
      data-testid={`fractal-row-${row.horizon}-${row.createdAt}`}>
      <td className="py-2 px-3 tabular-nums" style={{ color: '#64748b', fontSize: 12 }}>
      ...
      <td className="py-2 px-3 text-right tabular-nums" style={{ color: deltaColor }}>
        {deltaEntry != null
          ? `${deltaEntry >= 0 ? '+' : ''}${(deltaEntry * 100).toFixed(2)}%`
          : '—'}
      </td>
